...
Figure \ref{fig:designout} contains the best designs found using both objective functions. When using the mean kriging variance ($\overline{Q}_{puk}$), the best design spreads nodes of the network fairly evenly throughout the spatial domain. With the maximum kriging variance ($Q^*_{puk}$), the best design is more erratic. Notably, there are many more nodes directly on or very close to the border of the spatial domain. Further, interior areas have a tendency to be more sparsely populated with nodes. This is unsurprising since $Q^*_{puk}$ is worst case kriging variance over the entire county, which incentivizes putting monitoring locations near the edges of the county for two reasons. First, a point near the center of the county is likely to have monitoring locations all around it so that it is relatively easy to predict DM8 there, while a point near an edge of the county will only have monitoring locations to one side of it. Putting more monitoring locations near the edges of the county mitigates this. The second and probably most important reason is that the farther away a point is from other monitoring locations, the more the prediction depends on the model's estimate of $\bm{\beta}$ and the less it depends on the values of nearby observations. So $Q^*_{puk}$ places a premium on a better estimate of $\bm{\beta}$, and putting more monitoring locations near the edges of the county results in smaller variances for the elements of $\widehat{\bm{\beta}}_{gls}$.

\section{Discussion}\label{sec:discuss}

In this paper we reviewed PSO algorithms, introduced AT-PSO algorithms, and demonstrated that AT-PSO and PSO algorithms more generally are useful for spatial design problems. In the simulation study in Appendix \ref{app:psocompare} we found that AT-BBPSO variants were especially attractive for optimization problems with complicated objective surfaces potentially with many local optima. In Section \ref{sec:houston} and simluations not reported here we have found that spatial design problems typically are not this difficult in the sense that AT-BBPSO variants are no longer clearly better than other PSO alternatives. Similarly, the global neighborhood topology seems to be the best for the spatial design problem we considered, which is another symptom that the objective surface is not as complicated as some of the test functions from Appendix \ref{app:psocompare}. In this context the standard PSO algorithms with the global neighborhood do very well, though AT-PSO with the right configuration appears to be slightly better. The AT-BBPSO algorithms and GAs are comparable to both PSO and AT-PSO, though again the best performing AT-PSO algorithms still appear to outperform them.

The upshot is that at least for spatial design problems similar to the one we considered here, using a standard PSO algorithm with standard parameter settings and the typical modifications we discussed in Appendix \ref{subapp:pso} is a good default, especially since this algorithm is already implemented in several widely available packages. Similarly, standard GAs are a good default option, and packages for implementing them are also widely available. Further, in spatial design problems which are much lower dimensional than the one discussed in this paper, we have found that standard PSO algorithms are often the best. However, our results in Appendix \ref{app:psocompare} suggest that in more complex spatial models, e.g. with a more complex mean function, the objective surface may be complex enough that there are significant benefits to using AT-BBPSO. As long as the user has a reasonable expectation that the objective surface is not too complex, whichever algorithm is easier for them to use and implement is likely the best. While the design problem we discuss does not illustrate the advantages for AT-BBPSO, we prefer its simplicity for demonstrating the use of PSO algorithms more generally.

The usefulness of PSO and AT variants for statistics is not limited to spatial design. Other design problems are good candidates, but in general any optimization problem which is difficult to solve with gradient based algorithms is a good candidate, especially when near-optimal solutions are tolerable. In the spatial design literature exchange algorithms are commonly employed instead of PSO or GAs, though these require limiting the search space to a discrete grid \citep{nychka1998design,wikle1999space,wikle2005dynamic}. PSO (and GA) allow the entire search space to be used without restriction, though in cases where there is a known discrete set of possible monitoring locations the exchange algorithm is more suited to the task.
