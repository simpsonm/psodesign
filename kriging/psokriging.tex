...
Any PSO variant can also be combined with various neighborhood topologies that control how the particles communicate with each other. The default global topology allows each particle to see each other particle's previous best location for the social components of their respective velocity updates, but this can cause inadequate exploration and premature convergence. Alternative neighborhood topologies limit how many other particles each particle can communicate with. We use a the global topology for our spatial design problem since it yielded the best performing PSO algorithms in this context, though see Appendix \ref{app:psodetail} for a brief description of a common alternative called the stochastic star topology; \cite{miranda2008stochastic}.
