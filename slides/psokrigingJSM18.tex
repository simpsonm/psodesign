...
See e.g. \citep{clerc2011spso}, \citep[][appendix]{simpson2017adaptively}.
\begin{itemize}
...
\item Bare-bones PSO (no velocity term)
\end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{Adaptively Tuned Bare Bones PSO and Results}
AT-BBPSO strips away the velocity term (typically $df = 1$):
\begin{align*}
...
\end{align*}
\pause
Intuition: tuning $\omega(k)/\sigma^2(k)$ allows the swarm to adjust the exploration / exploitation tradeoff based on local conditions.
\begin{itemize}
...
\item Minimize $Q_{puk}^*(\bm{d}) = \max_{i=1,2,\dots,N_t}\sigma_{puk}^2(\bm{r}_i)$
\end{itemize}
