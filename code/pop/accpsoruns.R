...
for(id in 1:max(accpsoout$psoid)){
  ...
  if(as.character(ranef) == "iid"){
    idelta <- which(ndeltasiid == ndelta)
  } else if(as.character(ranef) == "full"){
    idelta <- which(ndeltasfull == ndelta)
  }
  nell <- ndelta*(ndelta + 1)/2
  datlist <- switch(as.character(ranef), iid = datlistiids[[idelta]], full = datlistfulls[[idelta]])
  lpost <- switch(paste(model, ranef, sep=""),
                  poisiid = poislpostiid,
                  lnormiid = lnormlpostiid,
                  poisfull = poislpostfull,
